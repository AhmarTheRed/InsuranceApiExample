...
{
    [ApiController]
    [Route("api/clients/{clientId}/policies/{policyId}/claims")]
    public class ClaimsController : ControllerBase
    {
        ...
        public ClaimsController(ILogger<PoliciesController> logger, IClientRepository clientRepository, IPolicyRepository policyRepository, IClaimRepository claimRepository)
        {
            _logger = logger;
        ...
        [HttpPut("{id}")]
        public async Task<IActionResult> Put([FromRoute] Guid clientId, [FromRoute] Guid policyId, Guid id, [FromBody] Claim claim)
        {
            if (await _clientRepository.GetClient(clientId) == null || await _policyRepository.GetPolicy(policyId) == null)
                return NotFound();
        ...
        [HttpPost]
        public async Task<IActionResult> Post([FromRoute] Guid clientId, [FromRoute] Guid policyId, [FromBody] Claim claim)
        {
            if (await _clientRepository.GetClient(clientId) == null || await _policyRepository.GetPolicy(policyId) == null)
                return NotFound();

            var addedClaim = await _claimRepository.AddClaim(policyId, claim);
            return CreatedAtAction(nameof(Get), new { clientId = clientId, policyId = policyId, id = addedClaim.Id }, addedClaim);
        }
